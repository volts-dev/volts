...
func (s *Server) recv(c *net.UDPConn) {
	...
	for {
		...
		if err := s.parsePacket(buf[:n], from); err != nil {
			log.Errf("[ERR] mdns: Failed to handle query: %v", err)
		}
	}
}
...
func (s *Server) parsePacket(packet []byte, from net.Addr) error {
	...
	if err := msg.Unpack(packet); err != nil {
		log.Errf("[ERR] mdns: Failed to unpack packet: %v", err)
		return err
	}
...
func (s *Server) probe() {
	...
	for i := 0; i < 3; i++ {
		if err := s.SendMulticast(q); err != nil {
			log.Errf("[ERR] mdns: failed to send probe:", err.Error())
		}
	...
	for i := 0; i < 3; i++ {
		if err := s.SendMulticast(resp); err != nil {
			log.Errf("[ERR] mdns: failed to send announcement:", err.Error())
		}
		select {
		case <-timer.C:
			timeout *= 2
